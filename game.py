...
game_dimensions = config.game_dimensions
in_a_row = config.in_a_row
move_amount = config.move_amount
...
def generate_game_state(node):
    ...
    while i < config.depth:
        ...
        if node.parent:
            node = node.parent
        i += 1
    board_history.append(np.array([[[None, 1, 0][node.player]] * game_dimensions[1]] * game_dimensions[0]))
    game_state = np.moveaxis(np.array(board_history), 0, -1)
    return game_state

def get_legal_moves(board):
    legal_moves = []
    for i in range(game_dimensions[1]):
        for i2 in range(game_dimensions[0]):
            if board[i + i2 * game_dimensions[1]] != 0:
                legal_moves.append(-1 if i2 == 0 else i + (i2 - 1) * game_dimensions[1])
                break
        else:
            legal_moves.append(i + i2 * game_dimensions[1])
    return legal_moves
